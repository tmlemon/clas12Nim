...
\begin{figure}
	...
	\caption{(Color Online) Top: the GEMC implementation of the SVT geometry. The three regions are shown in the sliced view. The silicon sensors are
...
\begin{figure}
	...
	\caption{(Color Online) Top: Process ID algorithm cartoon for SVT. The strip number is assigned based on the local position of the track
...
\begin{figure}[t]
	...
	\caption{(Color Online) The SVT hit sharing algorithm. If a step in a given strip happens within 39 $\mu$m of the adjacent strip, then
...
\begin{figure}
	...
	\caption{(Color Online) The occupancy in the various SVT layers for different thresholds for one event containing a proton track (direction indicated
             ...
             the remaining panels where the condition of some energy deposition is required. Top right: 30 keV energy threshold. Bottom right:
             with 40 keV energy threshold, only the proton track hits and a handful of background hits are visible.}
	\label{fig:radStudyThreshold}
\end{figure}


\noindent The radiation dose and the 1 MeV neutron equivalent damage was estimated. Most of the radiation
