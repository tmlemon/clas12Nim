...
\begin{tabular}{||c|c|c||} \hline \hline
   {\bf Goal}         & {\bf Physics Specification} & {\bf Design Spec or Feature}\\ \hline
Measure $\Gamma_v$  & $d \theta < 1mrad$   & planar chambers \\ 
accurately  & $dp/p < 1\% $ & identical cells (each superlayer)  \\ \hline
Select an exclusive  & $dp < .05 GeV/c$ & $250~\mu$m  accuracy      \\ 
reaction; e.g. only one    & $ p~\delta\theta < .02 ~GeV/c$ &  $+/- 6^\circ$ stereo angle  \\ 
missing pion       & $p ~sin\theta \delta \phi < .02 GeV/c$ & \\ \hline
