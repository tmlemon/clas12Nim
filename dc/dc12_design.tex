...
\begin{tabular}{||c|c||} \hline \hline
   {\bf Goal}         & {\bf Specification} \\ \hline
measure $\Gamma_v$  & $d \theta$ < 1mrad    \\ 
accurately  & dp/p < 1\%    \\ \hline
Select an exclusive  & dp .05 GeV/c       \\ 
reaction; e.g. only one    & $\delta\theta$p < .02 GeV/c          \\ 
missing pion       & $sin\theta \delta \phi p < .02 GeV/c$ \\ \hline
track resolution       & $dp/p < 0.3\% at 10 GeV/c, 7^{circ}$ \\ 
           & $dp/p < 1\%$ lower momenta, larger angles \\ \hline
small       & Luminosity = $10^{35}$~cm$^{-2}$s$^{-1}$  \\ 
cross sections  & high efficiency\\ \hline
good acceptance   & $\delta\phi = 50\% at 5^{circ}$\\ \hline
\end{tabular}
\caption{\small{Physics goals to general specifications}}
...
For the purposes of simulating 
