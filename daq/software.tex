...
\begin{figure}[hbt]
	...
	\caption{Readout Controller (ROC) Diagram. ROC is the program typically running on 4-core VME Intel Controller. It includes 3 main threads performing  VME readout, data processing, and networking.}
...
\begin{figure}[hbt]
	...
	\caption{Event Builder (EB) Diagram. EB is the program typically running on multi-core Linux server. It collects event fragments from ROCs, build events, and sends events to the Event Transfer system.}
...
\begin{figure}[hbt]
	...
	\caption{Event Transfer (ET) System Diagram. It is the program typically running on multi-core Linux server. It accepts events from Event Builder and pass them through the set of stations allowing attached programs to perform data processing. Last station typically have Event Recorder attached to it.}
...
\begin{figure}[hbt]
	...
	\caption{Event Recorder Diagram. It is the program typically running on multi-core Linux server. It receives events from Event Transfer System and write them to the disk. It is possible to write several files in parallel to increase writing performance, event order is perserved in a process.}
...
\begin{figure}[hbt]
	...
	\caption{Runtime Database Web Browser example. In particular it allows to search for run(s) with certain parameters.}
	\label{fig:rcdb}
\end{figure}

\subsection{Online Data Monitoring}

The CLAS12 online data monitoring is the set of programs attached to Event Transfer System and processing data in real time. One of such program's output is shown on Fig.\ref{fig:online_monitor}. Monitoring system allows shift takers to insert histograms into the electronic logbook. Every CLAS12 detector has designated set of histograms which is closely monitored by shift takers and detector experts to ensure data quality.

\begin{figure}[hbt]
	\centering
	\includegraphics[width=1.0\columnwidth,keepaspectratio]{img/online_monitor.png}
	\caption{Online monitoring example. Only small porsion of the histograms is shown, full set includes hundreds of histograms used by shift takers to monitor various detectors and subsystems.}
	\label{fig:online_monitor}
\end{figure}

\subsection{Electronic Logbook}

The CLAS12 shift personal is using JLab Electronic Logbook System. This web-based system provides an interface to browse, search, and create electronic logbook entries. The system has been built using the Drupal content management system and utilizes a MySQL back-end database to store its entries. The system has been designed to consolidate and replace the functionality of the multiple electronic logbook systems that existed at Jefferson Lab between 1996 and 2012 \cite{logbook-ref}.
...
\begin{figure}[hbt]
	...
	\caption{CED Event Example. It includes schematic view of all detectors with reasonably realistic geometry.}
	\label{fig:ced}
\end{figure}
