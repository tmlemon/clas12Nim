...
\begin{figure}[hbt]
	...
	\caption{Configuration file example. Flat text configuration files were used for all DAQ and Trigger Components. Download and Upload procedures were implements, with uploaded settings being stored into data files and databases. Used format allows personal with appropriate training to understand and modify system settings using text editors. Version control is inforced using Github.}
	\label{fig:config}
\end{figure}
...
\begin{figure}[hbt]
	...
	\caption{CTOFxFT Delay Scan. In this scan FT trigger time was fixed as having smallest jitter, and CTOF trigger delay was changing. Similar delay scans were measured for all detectors participating in the trigger logic, making sure all trigger componets are inline in time.}
