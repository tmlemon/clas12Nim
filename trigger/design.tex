...
After segment search complete and six 112-bit masks are ready second step is performed. On that step pre-loaded road dictionary is used to identify possible track candidates (so-called road finding). Road disctionaries can be generated by Monte Carlo program or taken from the real data, both methods were used as described in \ref. At least five out of six superlayers are required to satisfy trigger condition. All found roads are reported to the Stage 2 every 16 ns. Information reported in form of 112-bit words used for geometry match on Stage 2.
