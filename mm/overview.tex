...
(JLab) facility (CLAS12)~\cite{nim:clas12} includes two tracking detector systems. A Silicon Vertex Tracker (SVT)
\cite{nim:svt} consists of a polyhedral arrangement around the target and resides in a 5~T solenoid magnetic field to
detect particles emitted between 40$^\circ$ and 130$^\circ$ with respect to the beam direction. The forward tracking
system consists of drift chambers~\cite{nim:dc} placed before, within, and after a toroidal magnetic field and covers
...
detectors called the Central Time-Of-Flight (CTOF)~\cite{nim:ctof} and the Central Neutron Detector (CND)~\cite{nim:cnd}
