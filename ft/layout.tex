...
electromagnetic shower (Moliere radius) to contain the shower produced by incident electrons to few readout cells,
...
PANDA-EMC), matched to large-area photosensors to obtain a factor of four more light per MeV of deposited energy than the original
CMS-ECal crystals.
...
(SICCAS). Since the light yield ($LY$) increases while lowering the temperature $T$ according to the law
...
The FT-Cal uses $10 \times 10$~mm$^2$ (model Hamamatsu S8664-1010) LAAPDs to read out the PbWO$_4$ scintillation
...
maximum effect. Each crystal is equipped with a separate LED, located on the upstream face, at the
...
The FT calorimeter has been designed to operate between 0$^\circ$C to room temperature. The FT-Cal cooling is
...
Each layer of the FT-Hodo comprises of 44 15~mm$\times$15~mm (P15) and 72 30~mm$\times$30~mm (P30)
...
With the typical maximum doses determined through Geant4 simulations with realistic beam and target parameters
...
\caption{Schematic of a single channel of the amplifier board for SiPM.} 
...
gain.{\color{green} The HV distribution to the groups of 8 SIPM is implemented on the mezzanine boards that also host a compensation circuit that allows for the independent regulation of each SIPM bias voltage up to a maximum of 0.4 V.} The low voltage system used for the FT-Hodo is the same as the one used for FT-Cal. Controls of both the HV
