...
\author{V.D. Burkert, L. Elouadrhiri, D. Anderson, S. Aune, H. Avakian, N. Baltzell, M. Battaglieri,  V. Baturin,
  ...
  Y. Sharabian, L.C. Smith, D. Sokhan, S. Stepanyan, M. Ungaro, A. Vlassov, D. Watts, L. Weinstein, C. Wiggins, A. Yegneswaran, 
...
\caption{(Color Online) The CLAS12 detector in the Hall~B beam line. The electron beam enters from the right and impinges on
...
in the Forward Detector (FD) consisting of the HTCC (yellow) with full coverage in polar
angle $5^\circ \le \theta \le 35^\circ$ and $\Delta \phi = 2\pi$ coverage in azimuth. The HTCC is followed by the
Torus magnet (grey), the drift chamber tracking system (light blue), another set of Cherenkov counters (hidden),
...
Silicon Vertex Tracker (hidden), which is surrounded by a Barrel Micromesh Tracker (hidden), the Central Time-of-Flight
counter and the Central Neutron Detector (pmts in blue)). At the upstream end, a temporarily installed back angle neutron detector (red) is also visible. In the operational configuration, the entire CLAS12 detector extends for 13~m along the
beamline.} 
\label{clas12}
\end{figure*}
...
field, and are then guided into a shielding pipe made from bulk tungsten material where they dump their energy. 
...
central cold hub on a common stainless steel cylinder, which also provides the geometrical symmetry for the alignment of
the coils near the magnet center (see Fig.~\ref{coil-mount}). This increases placement accuracy of the coil packages in areas
where the magnetic field is expected to be maximal. A full view of the assembled Torus coils and cryostat is shown in
Fig.~\ref{clas12-magnets}(left). The open range in azimuthal angle depends on the polar angle of the particle trajectory,
and ranges from 50\% of $2\pi$ at $5^\circ$ to about 90\% of $2\pi$ at $40^\circ$.


Each superconducting coil is made from a two-coil "double-pancake" potted in an aluminum case. The number of windings per
...
fifth coil is located outside of the 4 inner coils and generates a magnetic field in the opposite direction of the field of the 4 inner
...
$(2 \times 840 + 2 \times 1012)$, and in the shield coil 1392. The magnet is powered at a nominal currents of 2416~A.
 At
full current the solenoid generates a 5~T magnetic field at its center. The integrated field length along the magnet center is $\int Bdl = 7.0$~Tm, generating a stored energy of 20~MJ.The magnet has an inner warm bore of 78~cm
...
The six coils of the
Torus magnet mechanically support the forward tracking system, which consists of three independent drift chambers in
...
consists of 6 layers with wires strung at $\Delta{\phi} = 6^\circ$ from the normal to the sector mid-plane and 6 layers with wires
...
The LTCC system is part of the CLAS12  Forward Detector and is used for charged pion detection at
...
shields. The LTCC boxes are filled with heavy $\rm C_4F_{10}$ radiator gas, providing pion identification 
for momenta greater than 3.5~GeV.  The LTCC system has previously been used to detect electrons in the CLAS detector at
...
counters. The time-of-flight resolution of the scintillators is no longer sufficient to detect kaons at momenta 
greater than 3 GeV. For that purpose
...
The PCAL and EC are both sampling calorimeters consisting of six modules. Along the direction from the target, the EC consists of two parts, read out separately, called EC-inner and EC-outer. They 
provide longitudinal sampling of electromagnetic showers as well as of hadronic interactions. 
This improves identification of hadronic and electromagnetic interacting particles. 
Each module has a triangular shape with 54 (15/15/24,
...
view, providing spatial resolutions of less than 2~cm for energy clusters. The light from each scintillator readout group is routed to
...
\caption{(Color Online) Central Vertex Tracker, fully assembled with the SVT, BMT, and FMT. 
The BMT and FMT are shown on the
outside. The FMT has a circular opening in the center for the electron beam to path through. 
The SVT is encapsulated and hidden from view.}
\label{CVT-FMT}
\end{figure}
...
charged particles scattered from the production target, which is centered within the Solenoid magnet. Details of the tracking system are shown in Fig.~\ref{CVT}. It consists of two
...
tracking efficiency. Each layer is arranged azimuthally in 3 sectors of 120$^\circ$ azimuthal coverage. The system
...
for forward scattered charged particles. The fully assembled CVT, including the FMT, are shown 
in Fig.~\ref{CVT-FMT}.  See Ref.~\cite{BMT} for details on the BMT and on the FMT. \footnote{The FMT tracker has not been implemented during the experimental runs covered in this paper.}  
...
around the target and the CVT as seen in Fig.~\ref{ The barrel is aligned with the beam axis inside the 5~T Solenoid magnet. The 
...
Detector. Details are described in Ref.~\cite{CTOF}. Fig.~\ref{ctof} shows the CTOF system from the design
model and Fig.~\ref{ctof-cnd} shows the upstream end of the CTOF installed inside the Solenoid.
...
versus momentum are shown in Fig.~\ref{neg-pos}, and Fig.~\ref{vertex} shows the production vertex as reconstructed
...
detection elements and the hit pattern in the CVT, i.e. by a combination of the SVT and the BMT trackers. 
In contrast to the charged particle tracking in the FD that relies heavily on timing information for resolution, 
...
electrons) and their secondary interaction with beam line components. The production rate is orders of magnitude larger
than the hadronic production rate. These background sources have to be shielded through careful design of magnetic
channeling, as well as a proper design of the beam line shielding and the vacuum pipe to minimize interaction of these
...
electrons through the beam enclosure to avoid interactions with the beam line materials. Figure~\ref{gemc-event}  shows
...
increasing polar angles. The gaps between sectors are not due to inefficiencies of the HTCC detector, but the result
...
between the CD and the entrance to the drift chamber system. The reconstructed electron coordinates at the HTCC are 
shown in Fig.~\ref{\label{htcc-electrons-dis}, exhibiting a very uniform distribution in azimuthal angle. The
electron detection efficiency in the 48 segments of the HTCC that cover the entire azimuthal angle range is shown in Fig.~\ref{htcc-efficiency}. 

The coordinates of the reconstructed electrons at the front face of the PCAL are displayed in Fig.~\ref{electrons-xy}. 
...
Elastic scattering  of electrons on protons allows for the establishment of any deviations from the ideal
...
\caption{ Reconstructed {$\rm Mass^2$}versus momentum of positively charged particles.  
The graph contains a threshold on the minimum number of events to highlight the 
particle profile: bottom: $\pi^+$, mid: $K^+$, top: protons. The centroids of each particle distribution are 
independent of the momentum. Masses are computed from particle path length and from time-of-flight, 
Any momentum dependence would indicate systematics in the timing calibration or in the path length determination.  }
\label{mass2-mom}
\end{figure} 
...
shown in Fig.~\ref{mass2-mom}.  An overview of the particle identification in the various CLAS12 detector systems vs.
...
\caption{(Color Online) {$\rm Mass^2$} of positively charged particles evaluated from their 
path length and the time-of-flight information
...
4.5$^\circ$. Figure~\ref{gg-ecal} shows the invariant mass of the $\gamma\gamma$ system in the CLAS12 FD. The energy 
response of the FT to 2.2 GeV electrons, and the 2-gamma mass resolution are shown in Fig.~\ref{FT-en-gg). 
...
The CLAS12 data acquisition (DAQ) is designed for an average of 20~kHz level 1 (L1) 
...
Figure~\ref{trigger} shows one example of an electron triggered event with one additional positive charged track.   
...
acquisition system and the available computing resources. 
Details of the design, functionality, and performance of the CLAS12 trigger are
...
running the M\"oller polarimeter located upstream of the tagger magnet.
...
Other materials include solid nuclear targets of various kinds from $^{12}$C to $^{208}$Pb, dependent on
...
by  $\phi_p = \phi_e + 180^\circ$  can be exploited to understand potential relative alignment issues between FD and CD tracking. Figure~\ref{elastic-electrons} and Figure~\ref{CVT-elastic-protons} illustrate the correlation in elastic electrons detected in the FD sectors 
