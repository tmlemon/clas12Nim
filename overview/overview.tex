...
\caption{Vertex reconstruction in the FD. The upper row shows the reconstructed vertices in the production frame along
...
\caption{Left: Distribution of electrons at the HTCC location in polar and azimuthal angle. Bin sizes increase with
...
\caption{Inclusive $ep \to e'X$ coverage in $Q^2$ vs. $x_B$. The full kinematics is measured simultaneously. The
...
\caption{Velocity $\beta = v/c $ versus momentum of positively charged particles detected in the CLAS12 FD. The
...
\caption{Overview of particle identification in various detectors of the CLAS12 FD. The higher (lower) intensity
...
\caption{The left panels show the elastic peak at 10.6~GeV beam energy as measured in 3 representative sectors. The
...
\caption{Top panel: The invariant mass of two high-energy photons in sector 4 of the ECAL. The background beneath
...
\caption{View of an event in CLAS12 from the ced event display. Predefined trajectories are employed to select hit
patterns in the 3 drift chamber regions that correspond with hits in the FTOF and with localized energy deposition
...
the physics requirements. All targets are positioned inside CLAS12 using support structures that are inserted from
...
and the occupancy increased by 50\%.}
\label{occupancies1}
\end{figure}
...
CLAS12 is designed for opeation at a significantly higher luminosity compared to CLAS. However, the high luminosity
...
& Neutron det. (CD) &  $ 40^\circ \le \theta \le 125^\circ$ \\ \hline
...
& Azim. angles (FD) & $\sigma_\phi  \le 1$~mrad/$\sin\phi$ \\
& Azim. angles (CD) & $\sigma_\phi \le 1$~mrad \\
...
Particle ID& $\pi/K ~(3\sigma)$ & $ p \le 3.0$~GeV  \\
& $K/p ~ (3\sigma)$  & $ p \le 4.0$~GeV \\
& $\pi/p~ (3\sigma)$  & $ p \le 5.0$~GeV \\ \hline
